...
public class StoredBeanService {
	...
	/**
	 * インデックスが必要なプロパティ名のSetを帰す。変更可能。
	 */
	public Set<String> getIndexSet() {
		return indexSet;
	}
	/**
	 * インデックスが必要なプロパティ名のSetを設定。変更可能。
	 */
	public void setIndexSet(Set<String> set) {
		indexSet = set;
	}
	...
	protected Key puts(Transaction tx, Key key, StoredBean bean) 
		...
		setProperty(entity,CLASS, cls.getName());
		setProperty(entity,PATH, getPath(key));
		...
		for (int i=0; i<methods.length; i++) {
			...
			if (pname != null) {
				...
				if (val instanceof StoredBean) {
					...
					setProperty(entity,pname, subKey);
				} else if (val instanceof Collection) {
					Collection coll = putsCollection(tx, key, (Collection)val, pname);
					setProperty(entity,pname, coll);
				} else if (val instanceof byte[]) {
					setProperty(entity,pname, new Blob((byte[])val));
				} else {
					setProperty(entity,pname, val);
				}
			}
		}
		DS.put(tx, entity);
	...
	private void setProperty(Entity entity,String pname, Object val) {
		if (indexSet.contains(pname)) {
			entity.setProperty(pname, val);
		} else {
			entity.setUnindexedProperty(pname, val);
		}
	}
				...
				throws EntityNotFoundException, Exception
	{
		...
		return entity2bean(tx, entity);
	}
	protected StoredBean entity2bean(Transaction tx, Entity entity) 
				throws EntityNotFoundException, Exception
	{
	...
	public StoredBean entity2bean(Entity entity) 
			throws EntityNotFoundException, Exception
	{
		Transaction tx = DS.beginTransaction();
		return entity2bean(tx, entity);
