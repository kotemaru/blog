...
(function($, undefined) {
	...
	/**
	 * This function applies JQM data-role attribute.
	 * - JQM-1.0 / JQM-1.1  take a different step.
	 * @param $parent  template target jQuery object.
	 */
	function enhance($parent, ver) {
		if (JQM_VER.lastIndexOf(ver,0) != 0) return;
		if (JQM_VER == "1.0") {
			$parent.page();
		} else {
			//enhance11($parent);
			$elems = $parent.find("[data-role]");
			$elems.each(function(){enhance11($(this));});
		}
	}
	function enhance11($elem){
	...
	function _onLoadTempl(data, url, callback) {
		...
		for (var i=0; i<q.length; i++) {
			// TODO:暫定対処、タイミング依存バグ有り。    
			// callback時にscopeの初期化が終っていない可能性がある。    
			var callback = q[i].callback;
			if (callback == null) {
				callback = function($this) {
					$this.jqmdp().refresh();
				}
			}
			_template(q[i].$this, $t, callback);
		};
	}
